...
bcDialog.show = function (sURL, nWidth, nHeight) {
	...
	document.getElementById("bcDialogFrameBox").innerHTML = '<iframe src="blank.html" id="bcDialogFrame" style="position:absolute; top: 0px; left: 0px;" frameBorder="0" scrolling="no">';
	...
	document.getElementById("bcDialogFrame").style.top	= document.body.parentNode.scrollTop + (pageSize[3] - nHeight) / 2+"px";
	document.getElementById("bcDialogFrame").style.width = nWidth+"px";
	document.getElementById("bcDialogFrame").style.height = nHeight+"px";
	document.getElementById("bcDialogFrame").src = sURL;
...
bcDialog.reSize = function (nWidth, nHeight) {
	...
	document.getElementById("bcDialogFrame").style.top	= document.body.parentNode.scrollTop + (pageSize[3] - nHeight) / 2;
	document.getElementById("bcDialogFrame").style.width = nWidth+"px";
	document.getElementById("bcDialogFrame").style.height = nHeight+"px";
}
...
function BusinessCard() {
			...
			<div style="position:absolute;left:0px;top:0px;display:none;" id="BusinessCardDIV" onclick="bc.rtn(event)" class="BusinessCardDIV">\
				...
				<div style="position:absolute; left: 0px; top: 0px;">\
					...
					<div style="position: absolute; left: 10px; top: 10px;"><a id="BusinessCardPhotoLink" target="_blank" onclick="bc.createLog(\'headpic\');" href="javascript:void();"><img src="http://image2.sina.com.cn/blog/tmpl/v3/images/default_icon.jpg" alt="用户头像" width="60" height="60" id="BusinessCardPhoto" border="0"/></a></div>\
						<!--头像底部链接-->\
						<div style="position: absolute; left: 10px; top: 74px; width: 60px; height: 14px; text-align: center; line-height: 14px;"><a href="javascript:void(0);" onclick="bc.createLog(\'home\');" target="_blank" id="BusinessCardHome">进入主页</a></div>\
						<!--my2008链接-->\
	...
	this.show = function (u, e, f, a) {
		...
		if(f && f.frameElement) {
			...
			fT = pos[1] - document.body.parentNode.scrollTop;
		...
		bcDIV.style.top = (y + document.body.parentNode.scrollTop + 3) + "px";
...
//				bcQ.href = "http://q.sina.com.cn/mygroup.php?uid=" + udata["uid"];
				...
				bcSED.onclick = function () {
					...
					if(showMode) {
						bcDialog.show2("http://my.blog.sina.com.cn/myblog/message/send_message.php?domain=1&toid=" + udata["uid"], 278, 100);
						bc.hidden();
					}
					else {
						bcDialog.show("http://my.blog.sina.com.cn/myblog/message/send_message.php?domain=1&toid=" + udata["uid"], 278, 100);
						bc.hidden();
					}
				}
				bcFLD.onclick = function () {
					...
					if(showMode) {
						...
						bcDialog.show("http://my.blog.sina.com.cn/friend/add_friend.php?opid=" + udata["uid"], 278, 258);
						bc.hidden();
					}
				}
