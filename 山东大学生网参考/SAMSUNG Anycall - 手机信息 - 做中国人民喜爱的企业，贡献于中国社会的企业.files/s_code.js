...
// Because s_account is variable, get the country code.
...
function getCurrencyCode(siteCode) {
	...
	var arrSiteCode = new Array("ar", "au", "at", "be_fr", "be", "br", "ca_fr", "ca", "cl", "cn", "co", "cz", "dk", "ee", "fi", "fr", "de", "gr", "hk_en", "hk", "hu", "in", "id", "iran", "ie", "il", "it", "jp", "kz_ru", "sec", "lv", "lt", "my", "mx", "nl", "nz", "no", "latin", "pe", "ph", "pl", "pt", "ru", "sa_en", "sg", "sk", "za", "es", "se", "ch_fr", "ch", "tw", "th", "tr", "us", "ae", "ua", "ua_ru", "vn", "uk", "global");
	var arrCurrencyCode = new Array("ARS", "AUD", "EUR", "EUR", "EUR", "BRL", "CAD", "CAD", "CLP", "CNY", "COP", "CZK", "DKK", "EEK", "EUR", "EUR", "EUR", "EUR", "HKD", "HKD", "HUF", "INR", "IDR", "IRR", "EUR", "ILS", "EUR", "JPY", "KZT", "KRW", "LVL", "LTL", "MYR", "MXN", "ANG", "NZD", "NOK", "PAB", "PEN", "PHP", "PLN", "EUR", "RUB", "SAR", "SGD", "SKK", "ZAR", "EUR", "SEK", "CHF", "CHF", "TWD", "THB", "TRY", "USD", "AED", "UAH", "UAH", "VND", "GBP", "USD");
	var currencyCode = "";
...
var s_account = "samsungfccnweb";
var s = s_gi(s_account);

/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
/* E-commerce Config */
var site_cd = getSiteCode();
...
s.linkInternalFilters="javascript:,samsungmobile.com"
...
function s_doPlugins(s) {
    ...
    //internal Campaigns
	if(!s.eVar9) 
		s.eVar9=s.getQueryParam('intcmp');
...
s.visitorNamespace="samsungfunclub";
s.dc=112;
s.trackingServer="samsungfunclub.122.2o7.net";
...
// customized codes for sec.com start
	...
	// start of variables' definition.
		...
		var countryCode = getSiteCode();

	    String.prototype.trim = function() {
            return this.replace(/(^\s*)|(\s*$)/gi, "");
        }

		function splitString(str, split_item) {
			return this.arrOfStrings = str.split(split_item);
		}
		...
		function getTextNode(id) {
			...
			try {
				...
				var productName = target.getElementsByTagName("strong");
	
				str = hiers[1].innerHTML.trim().replace("＞", "");
	
				for( var i = 2 ; i < hiers.length ; i++ ) {
					str += ":" + hiers[i].innerHTML.trim().replace("＞", "");
				}
	
				if (productName != "" && productName != undefined && productName != null) {
					if( productName.length > 0 ) {
						productName =  productName[0].innerHTML.replace(/(^\s*)|(\s*$)/g, "");
						str += ":" + productName;
					}
				}
			} catch(e) {
		...
		function getProductName(id) {
			...
			target = target.getElementsByTagName("strong");

			if (target != "" && target != undefined && target != null) {
				if( target.length > 0 )  {
					return target[ target.length - 1 ].innerHTML.replace(/(^\s*)|(\s*$)/g, "");
				}
				else return false;
			}
		...
		function getLanguageCode(siteCode) {
			var arrLangCode = new Array("es", "en", "de", "fr", "nl", "pt", "fr", "en", "es", "cn", "es", "cs", "da", "et", "fi", "fr", "de", "el", "en", "zh2", "hu", "en", "id", "fa", "en", "en", "it", "ja", "ru", "ko", "lv", "lt", "en", "es", "nl", "en", "no", "es", "es", "en", "pl", "pt", "ru", "en", "en", "sl", "en", "es", "sv", "fr", "de", "zh", "th", "tr", "en", "en", "uk", "ru", "en", "vi", "en", "en");
			var arrSiteCode = new Array("ar", "au", "at", "be_fr", "be", "br", "ca_fr", "ca", "cl", "cn", "co", "cz", "dk", "ee", "fi", "fr", "de", "gr", "hk_en", "hk", "hu", "in", "id", "iran", "ie", "il", "it", "jp", "kz_ru", "sec", "lv", "lt", "my", "mx", "nl", "nz", "no", "latin", "pe", "ph", "pl", "pt", "ru", "sa_en", "sg", "sk", "za", "es", "se", "ch_fr", "ch", "tw", "th", "tr", "us", "ae", "ua", "ua_ru", "gb", "vn", "uk", "global");
		...
		var country = SplitedURL[1];
		//var req_newstype = gup("newstype");
		//var req_newsctgry = gup("newsctgry");
		//var req_newssubctgry = gup("newssubctgry");
		//var req_news_seq = gup("news_seq");
		//var req_group = gup("group");
		//var req_type = gup("type");
		//var req_subtype = gup("subtype");
		//var req_subsubtype = gup("subsubtype");
		//var req_model_cd = gup("model_cd");
		var flag = gup("flag");

		SplitedURL[ SplitedURL.length - 1 ] = SplitedURL[ SplitedURL.length - 1 ].split("#")[ 0 ];
		SplitedURL[ SplitedURL.length - 1 ] = SplitedURL[ SplitedURL.length - 1 ].split(";")[ 0 ];

		// valiable initalizing
		...
		s.prop7 = s.prop8 = s.prop9 = s.prop10 = "";

		// temporary code for country & language variables.
		s.prop2 = getSiteCode();
		s.prop1 = getLanguageCode(s.prop2);
	...
	///////////////////////////////////////////////////////////////////////////
	function setProductVars(pageName) {
		if (SplitedURL[1] != "") {
			s.pageName += SplitedURL[ 1 ];
		}
		
		if (SplitedURL[2] != "")  {
			s.pageName += ":" + SplitedURL[ 2 ];
			s.channel += SplitedURL[ 2 ];
			s.prop3 = s.channel;
			//s.prop7 += SplitedURL[ 2 ];
			//s.prop9 += SplitedURL[ 2 ];
			//s.prop10 += SplitedURL[ 2 ];
			s.hier1 += ":" + SplitedURL[ 2 ];
		
		} else {
			s.pageName += ":" + pageName;
			//s.prop8 += SplitedURL[ 1 ] + ":" + pageName + "_home";
			s.hier1 += ":" + pageName;
	...
	/**
	*	parsing the breadcrumb
	*/
    function set_vars()	{
    	
		var newPageName = getTextNode("pathindicator");
		// for the breadcrubm (pathindicator) pages
		if (newPageName != false) {
		
			s.pageName = "cn" + ":" + newPageName;
		
			s.pageName = s.pageName.replace("home:", "");
			s.prop2 = s.prop2.replace("home:", "");			
			
          	if (newPageName.split(":")[0] != undefined) {              
				s.channel = s.prop1 + ":" + newPageName.split(":")[0];
			    s.prop2 = s.prop1 +  ":" + newPageName.split(":")[0];
			}

			if (newPageName.split(":")[1] != undefined) {
                s.prop3 = s.prop1 + ":" + newPageName.split(":")[0] + ":" + newPageName.split(":")[1];
			}

			if (newPageName.split(":")[2] != undefined) {
                s.prop4= s.prop1 + ":" + newPageName.split(":")[0] + ":" +newPageName.split(":")[1]+ ":" + newPageName.split(":")[2];
				
			}

			if (newPageName.split(":")[3] != undefined) {
                s.prop4= s.prop1 + ":" + newPageName.split(":")[0] + ":" +newPageName.split(":")[1]+ ":" + newPageName.split(":")[3];
				s.pageName=s.prop4;
			}

			s.hier1 = s.pageName.replace( /:/gi, ":" ).replace(s.prop2+">home", s.prop2+":home");
			
			// for the main pages
			if (SplitedURL[3] == undefined) {
				s.pageName += ":home";
				s.channel += ":home";
				s.hier1 += ":"+"home";
			}
		...
		if( SplitedURL[ 2 ].toLowerCase() == "phone" )
		{
			if( SplitedURL[ 3 ].toLowerCase() == "view" )
			{
				if(SplitedURL[ SplitedURL.length - 1 ].replace( ".jsp", "" ) == "phone")
				{
					s.pageName += ":overview";
				    s.hier1 += ":overview";
					s.prop4= s.prop3+":overview";

				} else if (SplitedURL[ SplitedURL.length - 1 ].replace( ".jsp", "" ) == "value")
				{
					s.pageName += ":specification";
				    s.hier1 += ":specification";
					s.prop4= s.prop3+":specification";
				} 
				else if (SplitedURL[ SplitedURL.length - 1 ].replace( ".jsp", "" ) == "bigpic")
				{
					s.pageName += ":bigpicture";
				    s.hier1 += ":bigpicture";
					s.prop4= s.prop3+":bigpicture";
				} 
				else {
					s.pageName += ":" + SplitedURL[ SplitedURL.length - 1 ].replace( ".jsp", "" );
				    s.hier1 += ":" + SplitedURL[ SplitedURL.length - 1 ].replace( ".jsp", "" );
					s.prop4= s.prop3 + ":" + SplitedURL[ SplitedURL.length - 1 ].replace( ".jsp", "" );
				}
				//s.products = "Phone;" + countryCode + ":" + s.pageName.split(":")[3];
			}
	...
	*/	
	function init_vars() {
	     ...
	     //phone->view
		 if(SplitedURL[SplitedURL.length - 2].toLowerCase() ==  "view")
          {
				//s.events = "prodView,event2";
				//s.products = ";" + countryCode + ":" + deleteSpace(getProductName("pathindicator"));

				// phone->view->phone.jsp
				if (SplitedURL[SplitedURL.length - 1].toLowerCase() ==  "phone.jsp") {
					setProductVars("phone");
				// phone->view->bigpic.jsp
				} else if (SplitedURL[SplitedURL.length - 1].toLowerCase() ==  "bigpic.jsp") {
					setProductVars("bigpic");
				// phone->view->download.jsp
				} else if (SplitedURL[SplitedURL.length - 1].toLowerCase() ==  "download.jsp") {
					setProductVars("download");
				// phone->view->value.jsp
				} else if (SplitedURL[SplitedURL.length - 1].toLowerCase() ==  "value.jsp") {
					setProductVars("value");				
				} 		
		  }
		 //member->reg_time(join success)
		 else if (SplitedURL[2] == "member" && SplitedURL[3] =="reg_time.jsp")
		{          
			       s.pageName="cn:member:reg_time";
                   s.events = "event1";
				   s.channel += ":member";
				   s.prop2   += ":member";
				   s.prop3   += ":member:reg_time";
			
		}
			 //member->me->index(?flag=true,login success)
		else if (SplitedURL[2] == "member" && SplitedURL[3] == "me" &&SplitedURL[4] =="index.jsp")
		{
		...
		else if (SplitedURL[2] == "event" ||SplitedURL[2] == "support")
		{
            
		}
		else if (SplitedURL[2] == "index" )
		{
                s.pageName += "cn:home";
				s.channel += ":home";
				s.prop2   += ":home";
		...
		else if(SplitedURL[2] == "phone" )
		{
			 if(SplitedURL[3] =="index.jsp" )
			{
                s.pageName += "cn:phone";
				s.channel += ":phone";
				s.prop2   += ":phone";
				...
				if (SplitedURL.length > 4) {
					if (SplitedURL[3] != "") {
						s.pageName += ":" + SplitedURL[3];
						s.prop3 = s.prop2 + ":" + SplitedURL[3];
					}
				}
				
				if (SplitedURL.length > 5) {
					if (SplitedURL[4] != "") {
						s.pageName += ":" + SplitedURL[4];
						s.prop4 = s.prop3 + ":" + SplitedURL[4];
					}
				}
				

				if (SplitedURL.length >= 6) {
					for (var i=6; i <(SplitedURL.length-1); i++) {
						if (SplitedURL[1] == "global") {
							s.pageName += ":" + SplitedURL[i];
						} else {
							s.pageName += ":" +SplitedURL[i];
				...
				if( SplitedURL.length ==4 )
				{
						...
						s.channel += ":" + SplitedURL[2];
						s.prop2 = s.channel;
						s.prop3 = s.prop2+":"+SplitedURL[ SplitedURL.length - 1 ].replace( ".jsp", "" );
					}
				...
				/*
				s.prop7 = "";
				s.prop8 += SplitedURL[ 1 ] + ":" + SplitedURL[ SplitedURL.length - 2 ] + ":" + SplitedURL[ SplitedURL.length - 1 ];
				s.prop9 += ":" + SplitedURL[ 3 ];
				*/
				//if (SplitedURL.length > 5) s.prop10 = SplitedURL[ 2 ] + ":" + SplitedURL[ 3 ] + ":" + SplitedURL[ 4 ];
				...
				if (SplitedURL[SplitedURL.length - 1] == "index_main.html") {
					s.pageName += ":home";
					s.channel += ":home";
					s.hier1 = s.pageName.replace(/:/gi, ":");
					//s.prop8 = SplitedURL[ 1 ] + ":" + SplitedURL[ SplitedURL.length - 1 ];
				} else {
					s.pageName += ":intro";
					s.channel += ":intro";
					s.hier1 = s.pageName.replace(/:/gi, ":");
					//s.prop8 = SplitedURL[ 1 ] + ":intro"
				}
...
/* identifying name, server, and channel */
	...
	/*
	  1. Flex Example 
		(1) support search 
            1) function call
               ExternalInterface.call('supportSearch',searchBox.SearchText.text,'event1','model number'); 
			   ExternalInterface.call('s_control_click', 'products,events,eVar4', ';'+searchBox.SearchText.text+',event1,'+'model number', 'o' 'support search');
            ...
            2) setting call
		       ...
		       - search by category name 
					...
					s.products = ";" + productId;
					s.events = "event1";
					s.eVar4 = "product category";
					s.tl(this, "o", "support search");

	  2. Tagging Examples
	     (1) Dealer locator searches
			 s_control_click('products,events', 'event34', ';'+site_code+':'+Product_ID+',event34', 'o', 'Dealer locator searches'); 

			 s.linkTrackVars="products,events"
             s.linkTrackEvents="scAdd"
			 s.events="scAdd" 
			 s.products=";site_code:display name"
             s.tl(this, 'o', 'Dealer locator searches');


		 (2) add to cart 
			 s_control_click('products,events', 'scAdd', ';'+site_code+':'+Product_ID+',scAdd', 'o', 'add to cart'); 
		 ...
		 (3) buy online
		 	s_control_click('products,events', 'event32', ';'+site_code+':'+diaplay_name+',event32', 'o', 'buy online'); 
	...
	*/
	...
	function s_control_click(vLinkTrackVars, vLinkTrackEvents, vLinkTrackValues, vType, vTypeName) {
		...
		for(var xFI = 0; xFI < vSplitLinkTrackVars.length; xFI++) {
			if(vSplitLinkTrackVars[xFI] == "products") {
				eval("s." + vSplitLinkTrackVars[xFI] + "='" + deleteSpace(vSplitLinkTrackValues[xFI]) + "'");
			} else {
	...
	// Main Banner setting
	function setMainBanner(bannerId) {
		s.prop11 = countryCode + ":" + bannerId;
	}
	
	// Banner Setting
	function setBanner(vBannerId, vType, vTypeName) {
		s.linkTrackVars = 'prop11';
		s.linkTrackEvents = 'none';
		
		s.prop11 = countryCode + ":" + vBannerId;
	...
	function setProp(vPropNo, vPropValue, vLinkType, vLinkName) {
		s.linkTrackVars = vPropNo;
		s.linkTrackEvents = 'none';

		eval("s." + vPropNo + "='" + countryCode + ":" + vPropValue + "'");


		s.tl(this, vLinkType, vLinkName);
		s.linkTrackVars='none';
		s.linkTrackEvents='none';
	}